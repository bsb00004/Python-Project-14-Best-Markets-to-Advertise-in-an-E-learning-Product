...
# 
# - Split each string in the <span style='background :Pink'>JobRoleInterest</span> column to find the number of options for each participant.
...
# It turns out that only 31.7% of the participants have a clear idea about what programming niche they'd like to work in, while the vast majority of students have mixed interests. But given that website offer courses on various subjects, the fact that new coders have mixed interest might be actually good for us.
# 
# The focus of most courses is on web and mobile development, so let's find out how many respondents chose at least one of these two options.
...
# 
# It turns out that most people in this survey (roughly 86%) are interested in either web or mobile development. These figures offer us a strong reason to consider this sample representative for our population of interest. We want to advertise the courses to people interested in all sorts of programming niches but mostly web and mobile development.
# 
...
# The <span style='background :pink'>MoneyForLearning</span> column describes in American dollars the amount of money spent by participants from the moment they started coding until the moment they completed the survey. Our company sells subscriptions at a price of \$59 per month, and for this reason we're interested in finding out how much money each student spends per month.
# 
...
# 
# Let's start with creating a new column that describes the amount of money a student has spent per month so far. To do that, we'll need to divide the <span style='background :pink'>MoneyForLearning</span> column to the <span style='background :plum'>MonthsProgramming</span> column. The problem is that some students answered that they have been learning to code for 0 months (it might be that they have just started). To avoid dividing by 0, we'll replace 0 with 1 in the <span style='background :plum'>MonthsProgramming</span> column.
...
# We want to group the data by country, and then measure the average amount of money that students spend per month in each country. First, let's remove the rows having null values for the <span style='background :PeachPuff'>CountryLive</span> column, and check out if we still have enough data for the four countries that interest us.
